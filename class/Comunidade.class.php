<?php
 ...
 { 
      ...
      public function get_post($id){
        ...
        $query = $this->pdo->query("SELECT * FROM `post_comunidade` WHERE id='$id' ");
      ...
      public function remove_post($id){
          if($this->pdo->query("DELETE FROM `post_comunidade` WHERE id='$id'")){
             return true;
         }else{
             return false;
         }
       ...
       public function add_like($id){
         ...
         if($this->pdo->query("UPDATE `post_comunidade` SET `likes`='$likes' WHERE id='$id'")){
             return $likes;
         }else{
             return $post->likes;
         }
      ...
      public function get_posts($inicio,$fim){
          
        $query = $this->pdo->query("SELECT * FROM `post_comunidade` ");
        $fetch = $query->fetchAll(PDO::FETCH_OBJ);

        if(count($fetch)>0){
            
            $query = $this->pdo->query("SELECT * FROM `post_comunidade` ORDER BY id DESC LIMIT $inicio, $fim");
            $fetch = $query->fetchAll(PDO::FETCH_OBJ);
            return $fetch;
     ...
     public function get_posts_qtd(){
      ...
      public function post_push($autor,$content,$data){
        
        $query = $this->pdo->prepare("INSERT INTO `post_comunidade` (data,autor,content) VALUES (:data,:autor,:content) ");

 ...
 <?php
  ...
  $whatsapi_class = new Whatsapi();
   ...
   $v_device_zapi      = $whatsapi_class->verific_device($_SESSION['SESSION_USER']['id'],'Whatsgw');
        ...
        <div style="margin-bottom:10px;" class="col-md-8">
            <div class="btn-toolbar ">
              <div class="btn-group mr-3">
                ...
                <button <?php if($user->id_plano != 7){ echo 'onclick="location.href=\'cart?upgrade\'"';}else{ echo 'onclick="$(\'#modal_pareamento_zapi\').modal(\'show\');"'; } ?>  class="btn btn-outline-secondary" ><i class="fa fa-whatsapp" ></i> Api Patrão  <?php if($user->id_plano != 7){ echo '<i class="fa fa-star text-primary"></i>';} ?></button>
        ...
        <div class="row" >
            ...
            <div class="col-md-6" >
                 ...
                 <br />
                  ...
                  <span style="display:none;" class="text-info timeCounterClass">Tente novamente em <span style="display:none;" class="text-info timeCounterClass" id="timeCount">30</span> segundos</span>
                 </center>
            </div>
        </div>
   ...
   <div class="modal-content">
     ...
     <div class="modal-footer">
       ...
       function remove_api_zapi(){
        ...
        function initTime(){
            $(".timeCounterClass").show();
            $("#timeCount").html("30");
            var countTimer = setInterval(function(){
                var time = parseInt($("#timeCount").text());
                var newTime = time-1;
                $("#timeCount").html(newTime);
                if(time == 0){
                    clearInterval(countTimer);
                    $(".timeCounterClass").hide();
                }
            },1000);
        }
        
        function verifyConected(){
           var conetecV = setInterval(function(){
                
                var keydevice_zapi = $("#keydevice_zapi").val();
                $.post('../control/api-zapi/control.load_qr.php',{verifyConected:true,keydevice_zapi:keydevice_zapi},function(data){
                    if(data == '1'){
                        $("#textConected").html("Conectado!");
                        clearInterval(conetecV);
                    }else{
                        $("#textConected").html("");
                    }
                });

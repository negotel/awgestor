<?php
				...
				<ul class="<?php echo $ulClass; ?>">
					<?php foreach ($knowledges as $knw){
						...
						<li class="<?php echo $isStickyCheck && $knw->is_stickey=="Y"?" is-sticky ":"";?>">
							...
							<a class=" <?php echo $is_porpup?" popupform ":"";?>" <?php echo $is_porpup?'  data-effect="mfp-move-from-top "':"";?> <?php echo $is_blank?' target="_blank" ':'';?> href="<?php echo site_url("knowledge/details/{$knw->id}/{$knw->slug_id}");?>"><i class="fa fa-file-alt"></i> <?php echo $knw->title;?>
								<?php if($isViewCount){?>
									<span class="view-counter float-right"><i class="fa fa-eye"></i> <?php echo _n(str_pad($knw->v_count, 2,'0',STR_PAD_LEFT));?></span>
								<?php }
									if($isLikeCount){
										?>
										<span class="view-counter float-right"><i class="fa fa-thumbs-up"></i> <?php echo _n(str_pad($knw->l_count, 2,'0',STR_PAD_LEFT));?></span>
									<?php }
									if($isDislikeCount){
										
										?>
										<span class="view-counter float-right"><i class="fa fa-thumbs-down"></i> <?php echo _n(str_pad($knw->d_count, 2,'0',STR_PAD_LEFT));?></span>
									<?php }?>
							</a>
	...
	if (! function_exists ( "get_open_ticket_link_2020" )) {
		function get_open_ticket_link_2020($class="",$title="Open Ticket",$icon='fa fa-ticket'){
			...
			if(!empty($userdata) || $isDisableGuestPopup){
				...
				?>
                <a data-effect="mfp-move-from-top" class="btn btn-lg btn-theme-light popupformWR <?php echo $class;?> open-ticket"  href="<?php echo site_url('user/login-register');?>" >
	...
	if (! function_exists ( "get_iconpicker_2020" )) {
		function get_iconpicker_2020( $title, $name, $value = '', $input_class = '' ) {
			?>
            <button name="<?php echo $name; ?>" class="btn btn-secondary  bss-icon-picker-2020 <?php echo $input_class; ?>" data-placement="bottom"  data-icon="<?php echo $value; ?>" value="<?php echo $value; ?>" role="iconpicker"></button>
			<?php
		}
	}
	...
	if (! function_exists ( "get_input_image_2020" )) {
		function get_input_image_2020( $title, $name, $value = '', $placeholder='', $style='width:108px; height: 108px;', $group_class = 'col-sm', $input_class = '' ) {
            ...
            <div class="form-group <?php echo $group_class; ?>">
                <label class="col-form-label" for="<?php echo $name; ?>"><?php _e( $title ); ?></label>
                <div class="">
                    <img class="app-image-input img-thumbnail" data-name="<?php echo $name; ?>" src="<?php echo $value; ?>" style="<?php echo $style; ?>"/>
	...
	if (! function_exists ( "getAPIPostValue_2020" )) {
		function getAPIPostValue_2020($key,$isHtml=false) {
		    if($isHtml){
		        return AppSecurity::RawPostValue($key,Mapp_setting_api::GetSettingsValue('bss2020',$key));
            }
			return PostValue($key,Mapp_setting_api::GetSettingsValue('bss2020',$key));
		}
	}
	...
	if (! function_exists ( "get_ticket_list_2020" )) {
		 ...
		 * @return string
		 */
		function get_ticket_list_2020( $ticket_list,$no_ticket_msg=""){
                        ...
                        <li class="">
                            <div class="kn-title">
                                <h3 class="">
                                    ...
                                    <span class="kn-like float-right btn-v-dtls text-success"><a href="<?php echo site_url("ticket/details/{$ticket->id}");?>" class="btn btn-xs btn-theme-light"><i class="fa fa-eye"></i> <?php _e("View Details") ; ?></a></span>
                                </h3>
                            </div>
                            <div class="kn-details ticket-item">
                                <div class="row m-0">
                                    <div class="col-sm">
                                        <div class="row">
                                            <label class="col-sm-5 col-6"><?php _e("Ticket Track ID") ; ?></label>
                                            <div class="col-sm-7  col-6"><?php echo $ticket->ticket_track_id;?></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm">
                                        <div class="row">
                                            <label class="col-sm-5 col-6"><?php _e("Current Status") ; ?></label>
                                            <div class="col-sm-7 col-6"><?php echo getTextByKey($ticket->status,$mainticket_obj->GetPropertyOptionsTag("status"));?></div>
                                        </div>
                                    </div>
                                    <div class="col-sm">
                                        <div class="row ">
                                            <label class="col-sm-5 col-6"><?php _e("Opened On") ; ?></label>
                                            <div class="col-sm-7 col-6"><?php echo $ticket->opened_time!="0000-00-00 00:00:00"?get_user_datetime_default_format($ticket->opened_time):"-";?></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="row m-0">
                                    <div class="col-sm-8">
                                        <div class="row">
                                            <label class="ctg col-sm-2 col-6"><?php _e("Category") ; ?></label>
                                            <div class="col-sm-7  col-6"><?php if(!empty($ticket->cat_id)){?><a href="#"><?php echo Mcategory::getParentStr($ticket->cat_id);?></a><?php }?></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm">
                                        <div class="row ">
                                            <label class="col-sm-5 col-6"><?php _e($ticket->status=="C"?"Closed On":"Last Replied On") ; ?></label>
                                            <div class="col-sm-7 col-6"><?php echo $ticket->last_reply_time!="0000-00-00 00:00:00"?get_user_datetime_default_format($ticket->last_reply_time):"-";?></div>
                                        </div>
                                    </div>
                                </div>
		...
		function GetTicketReplyHTML2020($reply_object,$ticket_user_id="",$class=""){
			...
			if($reply_object instanceof Mticket_reply){
				...
				if(!empty($user)){
					...
					?>
                    <div id="id_<?php echo $reply_object->ticket_id."_".$reply_object->reply_id;?>" class="<?php echo $class; ?> card card-default app-panel-box mb-3 ticket-reply <?php echo ($user->type=="A" || $user->type=="S")?"admin-user":"";?>">
                                                    ...
                                                    <div class="card card-default payment-panel <?php echo $payment_obj->status=="A"?" paid-panel":"";?>">
                                                        <div class="card-header"><?php _e("Payment Added"); ?>
															<?php if(!$isAdmin && in_array($payment_obj->status,['P','F'])){?>
                                                                <a href="<?php echo site_url("ticket/ticket-payment/{$payment_obj->ticket_id}/{$payment_obj->reply_id}/{$payment_obj->id}");?>" class="payment-btn btn btn-xs btn-success float-right"><?php _e("Pay Now") ; ?></a>
															<?php }?>
                                                        </div>
                                                        <div class="card-body">
                        ...
                        <li class="<?php echo $knw->is_stickey=="Y"?" is-sticky ":"";?>">
                            <div class="kn-title">
                                <h3 class="m-0">
                                    ...
                                    <span class="kn-like ab-rt-1 text-success"><i class="fa fa-thumbs-up "></i> <?php _n(str_pad($knw->l_count, 2,'0',STR_PAD_LEFT));?></span>
                                </h3>
                            </div>

...
<div class="register-form col">
	...
	<div class="form-group">
		<label style="width: 100%"><?php _e("Password");?><a data-effect="mfp-move-from-top" tabindex="-1"	href="<?php echo site_url("user/forget");?>" class="popupform  float-right"><?php _e("Forgot password?");?></a></label>
			...
			<?php if($isEnableDefaultLogin){?>
				...
				if(count($providers)>0 || count($remoteLogins) >0){
					foreach ($providers as $provider=>$link){?>
						<a href="<?php echo site_url($link.$redirect_token);?>" class="btn m-b-5 btn-sm btn-default btn-<?php echo strtolower($provider);?>"><i class="fab fa-<?php echo strtolower($provider);?>"></i> <?php echo $provider;?></a>
					<?php }?>
				</div>
				<?php }
					}?>

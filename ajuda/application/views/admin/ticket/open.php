<div class="row">
	<div class="col-md-12">
		...
		<div class="panel panel-default app-panel-box">
			<div class="panel-heading"><?php _e("New Ticket Form"); ?></div>		  
		  <div class="panel-body">		  
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="control-label label-required col-md-4" for="user_email"><?php _e("Email Address"); ?></label>
                                <div class="col-md-8">
                                    <input type="Email" class="form-control" id="user_email"  value="<?php echo  PostValue("user_email");?>" name="user_email" placeholder="<?php _e("Email Address"); ?>" data-bv-notempty="true"
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="control-label label-required col-md-4" for="priroty"><?php _e("Priroty"); ?></label>
                                <div class="col-md-8">
			 ...
			 <div class="form-group">
		      	<label class="control-label label-required col-md-2" for="title"><?php _e("Ticket Subject"); ?></label>
		      	<div class="col-md-10">
		      ...
		      <?php if(Mapp_setting::GetSettingsValue("is_public_ticket")=="Y"){?>
		      	<div class="form-group">
			      	<label  style="line-height: 40px;" class="control-label label-required col-md-2" for="is_public"><?php _e("Public Ticket"); ?></label>
			      	<div class="col-md-10">
		      ...
		      <?php }?>
		      <div class="form-group">
		      	<label class="control-label label-required col-md-2" for="cat_id"><?php _e("Ticket Category"); ?></label>
		      	<div class="col-md-10">
		      		<?php 		      		
		      		$options_category=Mcategory::getCategoryListHtmlOptionArrayOnlyTicket('A');
		      		?>
			        ...
			        <?php
				        ...
				        foreach ($all_category_fields as $fld){
					        echo app_get_html_form_field($fld,"custom_","","",2,10);
				        ...
				        <?php
					        ...
					        foreach ($custom_fields as $fld_group){
						        foreach ($fld_group as $cfld){
							        if(trim($cfld->cat_id,', ')=="R"){
								        continue;
							        }
							        $groups_ct="grp-cat-".implode(" grp-cat-",explode(",",$cfld->cat_id));
							        echo app_get_html_form_field($cfld,"custom_","custom-field-group ".$groups_ct."  grp-".$cfld->id,"",2,10);
						        }
					        }
				        ?>
                    </div>
		     ...
		     if(!empty($uploaded_file_list) && count($uploaded_file_list)>0){
			       			...
			       			<div class="col-md-5 col-sm-6 col-xs-3">
			       			   <div class="row file-preview-img">
					           	...
					           	<div class=" col-sm-4 col-xs-12 text-right">
					           		<?php if(!$isImage){?>
					           	  <i class="fa <?php echo $iclass;?> pull-right" style="color:<?php echo $icolor;?>;"></i>
					           	  <?php }else{?>
					              <img  class="img-responsive pull-right" src="<?php echo base_url("ticket/ticket-tmp-img/{$file_session_id}/{$fl->name}");?>" alt="<?php echo $fl->old_name;?>" />
					              <?php }?>
					           	</div>
					           </div>
			       			</div>
		      ...
		      ?>
		       ...
		       <div id="main_file_btn" class="form-group app-file-main-container">
		       	...
		       	<button data-target="main_file_btn" data-clone-inc="true" class="add-cloner-button btn btn-xs btn-info m-t-15 "><i class="fa fa-plus-circle"></i> <?php _e("Add Another File") ; ?></button>

			<?php }?>

		  </div>
		  <div class="panel-footer text-center">
		  	<button type="submit" class="btn btn-success btn-lg"><i class="fa fa-ticket"></i> <?php _e("Create Ticket") ; ?></button>
		  </div>

			...
			foreach ($result as &$data){
                ...
                if($bkstatus!="D" || $admindata->grade==0) {
                    ...
                    if($data->id!=$admindata->id && $admindata->grade==0 && $bkstatus!="D" && ACL::HasPermission("admin/app-permission-confirm/archive-user")){
                        $data->action .= "<li><a class='ConfirmAjaxWR text-danger' data-msg='".__("Are you sure to archive? Archive is like as delete. If you want to do it for temporary, then set inactive status instead of archive. You cann&#96;t revert if you process it.")."' href='" . admin_url("app-permission-confirm/archive-user/" . $data->id) . "'><i class='fa fa-trash '></i> ".__("Archive")."</a></li>";
                    }

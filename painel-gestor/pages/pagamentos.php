    ...
    <div class="container">
      ...
      <div class="col-md-12">
         <?php if($user->id_rev == 0){ ?>
          <div class="col-md-6">
            <button onclick="location.href='cart';" type="button" class="btn btn-outline-success" name="button"><i class="fa fa-plus"></i> Nova fatura</button>
          </div>
     ...
     if($user->id_rev != 0){ ?>
         <div class="row text-center" style="margin-top:5px;" >
           <div class="col-md-12" >
               <h3>Seu painel vence  <?= $user->vencimento; ?>. <br /> Caso tenha vencido, entre em contato com seu revendedor e renove a licença</h3>
           </div>
               ...
               <table class="table table-striped table-sm">
                 ...
                 <tbody>
                     ...
                     if($list_faturas){
                       ...
                       while ($fatura = $list_faturas->fetch(PDO::FETCH_OBJ)) {
                         ...
                         switch ($fatura->status) {
                           case 'Pendente': $status = "<span class='badge badge-secondary' >$idioma->pendente</span>"; break;
                           case 'Aprovado': $status = "<span class='badge badge-success' >$idioma->pago</span>"; break;
                           case 'Devolvido': $status = "<span class='badge badge-danger' >$idioma->devolvido</span>"; break;
                           case 'Rejeitado': $status = "<span class='badge badge-danger' >$idioma->rejeitado</span>"; break;
                           case 'Análise': $status = "<span class='badge badge-warning' >$idioma->analise</span>"; break;
                           case 'Cancelado': $status = "<span class='badge badge-danger' >$idioma->cancelado</span>"; break;
                           case 'Mediação': $status = "<span class='badge badge-danger' >$idioma->mediacao</span>"; break;
                   ...
                   <tr>
                     ...
                     <td title="<?= $idioma->valor_fat_e; ?> <?= $fatura->valor; ?> <?= $moedaN->nome; ?>" > <?= $moedaN->simbolo; ?> <?= $fatura->valor; ?> </td>
                     <td title="<?= $idioma->data_fat_e; ?> <?= $fatura->data; ?> às <?= $fatura->hora; ?>" ><?= $fatura->data; ?> - <?= $fatura->hora; ?></td>
                     <td title="<?= $idioma->form_pay_fat_e; ?>  <?= $fatura->forma; ?>" ><?= $icon_f.' '.$fatura->forma; ?>  <?php if($fatura->comprovante != '0'){ echo '<a target="_blank" href="<?=SET_URL_PRODUCTION?>/comprovantes/'.$fatura->comprovante.'" ><i class="fa fa-file" ></i></a>'; } ?><?= $comp_link; ?></td>
                     <td title="<?= $idioma->status_fat_e; ?> <?= $fatura->status; ?>" ><?= $status; ?></td>
                     <td>
                       <?php if($fatura->status == "Pendente"){ ?>
                        <button id="btn_pay_<?= $fatura->id; ?>" onclick="init_pay_method(<?= $fatura->id; ?>,'<?= $fatura->moeda; ?>');" title="PAGAR" type="button" class="click_<?= $num_click; ?> btn btn-sm btn-success" name="button"><?= $idioma->pagar; ?></button>
                        <button onclick="$('#id_fat_promo').val(<?= $fatura->id; ?>);$('#modal_codigo_promo').modal('show');" class="btn btn-sm" <?php if($dark == 1){ echo 'style="color:#000;background-color:#fff;"'; }else{ echo 'style="color:#fff;background-color:#000;"'; }  ?>  ><i class="fa fa-ticket" ></i></button>
                       <?php } ?>
                     </td>
                   </tr>
...
<!--  Modal payments -->
<div class="modal fade" id="modal_payment_2" tabindex="-1" role="dialog" aria-labelledby="Titutlo_modal_payment_2" aria-hidden="true">
               ...
               <div class="col-md-12 margin">
                    ...
                    <div id="como_pagar"  class="row" >
                        <div class="col-md-2" ></div>
                            <div <?php if($user->af == 0 || $user->af == NULL){ ?> class="col-md-4" <?php }else{ ?>  class="col-md-12  text-center" <?php } ?> id="mercadopago_div" >
                                <button onclick="meio_pay('mp');" id="btn_mp_pay" class="btn btn-lg btn-outline-secondary" >Mercado Pago <i class="fa fa-handshake-o" ></i></button>
                            </div>
                    ...
                    <div style="display:none;" id="row_ted"  class="row" >
                        <div style="margin-bottom:10px;" class="col-md-12">
                            <button class="btn btn-secondary btn-sm" onclick="$('#row_ted').hide();$('#como_pagar').show(100);" > <i class="fa-arrow-left fa"></i> Voltar</button>
                        </div>
                        ...
                        <div class="col-md-6" >
                            ...
                            <hr>
                             <ul>
                                <li>
                                    Banco: <b>260</b> Nu Pagamentos S.A. (Nubank)
                                </li>
                                <li>
                                    Agência: <b>0001</b>
                                </li>
                         ...
                         <div class="col-md-6" >
                          ...
                          <p>
                              <form id="form_comp" action="" method="POST" enctype="multipart/form-data">
                                  <label class="btn btn-outline-primary btn-sm " id="btn_comp" for="comprovante">Enviar comprovante</label>
                                  <input onchange="send_comp();" type="file" style="display:none;" name="comprovante" id="comprovante" />
                                  <input type="hidden" id="meio_pay_idFat" name="meio_pay_idFat" value="" />
                              </form>
                          </p>
                          <p>Precisa enviar o comprovante pelo celular ? Escaneie o QrCode Abaixo ou digite o endereço na barra de pesquisa.</p>
                           <p id="dados_mobile"></p>
                        </div>
...
<!--  Modal del clientes -->
<div class="modal fade" id="modal_payment_paypal" tabindex="-1" role="dialog" aria-labelledby="Titutlo_modal_payment_paypal" aria-hidden="true">
 <div class="modal-dialog" role="document">
   <div class="modal-content">
     <div class="modal-header">
       <h5 class="modal-title" id="Titutlo_modal_payment_paypal"><?= $idioma->ta_quase_la; ?> <?= explode(' ',$user->nome)[0]; ?></h5>
       <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
         <span aria-hidden="true">&times;</span>
       </button>
     </div>
...
<!--  Modal del clientes -->
<div class="modal fade" id="modal_codigo_promo" tabindex="-1" role="dialog" aria-labelledby="Titutlo_modal_codigo_promo" aria-hidden="true">
 <div class="modal-dialog" role="document">
   <div class="modal-content">
     <div class="modal-header">
       <h5 class="modal-title" id="Titutlo_modal_payment"><?= explode(' ',$user->nome)[0]; ?>, utilize um código promocional.</h5>
       <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
         <span aria-hidden="true">&times;</span>
       </button>
     </div>
          ...
          <div class="row">
           ...
           <div class="col-md-12 text-center margin">
            ...
            <div class="form-group">
                ...
                <input type="text" class="text-center form-control" style="text-transform: uppercase;" placeholder="Digite aqui" name="codigo_promo" id="codigo_promo" />
            </div>

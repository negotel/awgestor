    ...
    <nav aria-label="breadcrumb">
        <small>
            <ol class="custom-breadcrumbs">
                ...
                <li>
                    <a href="<?= url('heartbeat/' . $data->heartbeat->heartbeat_id) ?>"><?= language()->heartbeat->breadcrumb ?></a><i class="fa fa-fw fa-angle-right"></i>
                </li>
                <li class="active" aria-current="page"><?= language()->heartbeat_update->breadcrumb ?></li>
            </ol>
        </small>
    </nav>

    <div class="d-flex align-items-center mb-2">
        <h1 class="h4 text-truncate mb-0 mr-2"><?= sprintf(language()->heartbeat_update->header, $data->heartbeat->name) ?></h1>
            ...
            <form action="" method="post" role="form">
                ...
                <div class="form-row">
                    <div class="form-group col">
                        ...
                        <input type="number" step="1" id="run_interval" name="run_interval" class="form-control" value="<?= $data->heartbeat->settings->run_interval ?>" />
                    </div>

                    <div class="form-group col">
                        <label>&nbsp;</label>
                        <select id="run_interval_type" name="run_interval_type" class="form-control">
                            <option value="minutes" <?= $data->heartbeat->settings->run_interval_type == 'minutes' ? 'selected="selected"' : null ?>><?= language()->global->date->minutes ?></option>
                            <option value="hours" <?= $data->heartbeat->settings->run_interval_type == 'hours' ? 'selected="selected"' : null ?>><?= language()->global->date->hours ?></option>
                            <option value="days" <?= $data->heartbeat->settings->run_interval_type == 'days' ? 'selected="selected"' : null ?>><?= language()->global->date->days ?></option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group col">
                        ...
                        <input type="number" step="1" id="run_interval_grace" name="run_interval_grace" class="form-control" value="<?= $data->heartbeat->settings->run_interval_grace ?>" />
                    ...
                    <div class="form-group col">
                        ...
                        <select id="run_interval_grace_type" name="run_interval_grace_type" class="form-control">
                            <option value="seconds" <?= $data->heartbeat->settings->run_interval_grace_type == 'seconds' ? 'selected="selected"' : null ?>><?= language()->global->date->seconds ?></option>
                            <option value="minutes" <?= $data->heartbeat->settings->run_interval_grace_type == 'minutes' ? 'selected="selected"' : null ?>><?= language()->global->date->minutes ?></option>
                            <option value="hours" <?= $data->heartbeat->settings->run_interval_grace_type == 'hours' ? 'selected="selected"' : null ?>><?= language()->global->date->hours ?></option>
                            <option value="days" <?= $data->heartbeat->settings->run_interval_grace_type == 'days' ? 'selected="selected"' : null ?>><?= language()->global->date->days ?></option>
                        </select>
                    </div>
                </div>

                <div class="form-group">
                    <label for="is_enabled"><?= language()->heartbeat->input->is_enabled ?></label>
                    <select id="is_enabled" name="is_enabled" class="form-control">
                        <option value="1" <?= $data->heartbeat->is_enabled ? 'selected="selected"' : null ?>><?= language()->global->yes ?></option>
                        <option value="0" <?= !$data->heartbeat->is_enabled ? 'selected="selected"' : null ?>><?= language()->global->no ?></option>
                    </select>
                    <small class="form-text text-muted"><?= language()->heartbeat->input->is_enabled_help ?></small>
                </div>
                ...
                <div class="tab-content my-3">
                    <div class="tab-pane fade" id="advanced" role="tabpanel" aria-labelledby="advanced-tab">
                        ...
                        <div <?= $this->user->plan_settings->email_notifications_is_enabled ? null : 'data-toggle="tooltip" title="' . language()->global->info_message->plan_feature_no_access . '"' ?>>
                            <div class="form-group <?= $this->user->plan_settings->email_notifications_is_enabled ? null : 'container-disabled' ?>">
                                ...
                                <select id="email_notifications_is_enabled" name="email_notifications_is_enabled" class="form-control">
                                    <option value="1" <?= $data->heartbeat->notifications->email_is_enabled == 1 ? 'selected="selected"' : null ?>><?= language()->global->yes ?></option>
                                    <option value="0" <?= $data->heartbeat->notifications->email_is_enabled == 0 ? 'selected="selected"' : null ?>><?= language()->global->no ?></option>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" id="webhook_notifications" name="webhook_notifications" class="form-control" value="<?= $data->heartbeat->notifications->webhook ?>" />
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" id="slack_notifications" name="slack_notifications" class="form-control" value="<?= $data->heartbeat->notifications->slack ?>" />
                        ...
                        <div <?= $this->user->plan_settings->twilio_notifications_is_enabled ? null : 'data-toggle="tooltip" title="' . language()->global->info_message->plan_feature_no_access . '"' ?>>
                            <div class="form-group <?= $this->user->plan_settings->twilio_notifications_is_enabled ? null : 'container-disabled' ?>">
                                ...
                                <input type="text" id="twilio_notifications" name="twilio_notifications" class="form-control" value="<?= $data->heartbeat->notifications->twilio ?>" />

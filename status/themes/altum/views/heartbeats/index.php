...
<div class="container">
    ...
    <div class="row mb-4">
        <div class="col-12 col-xl d-flex align-items-center mb-3 mb-xl-0">
            <h1 class="h4 m-0"><?= language()->heartbeats->header ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= language()->heartbeats->subheader ?>">
        ...
        <div class="col-12 col-xl-auto d-flex">
            <div>
                <?php if($this->user->plan_settings->heartbeats_limit != -1 && $data->total_heartbeats >= $this->user->plan_settings->heartbeats_limit): ?>
                    <button type="button" class="btn btn-outline-primary disabled" data-toggle="tooltip" title="<?= language()->heartbeat->error_message->heartbeats_limit ?>">
                        <i class="fa fa-fw fa-sm fa-plus"></i> <?= language()->heartbeats->create ?>
                    </button>
                <?php else: ?>
                    <a href="<?= url('heartbeat-create') ?>" class="btn btn-outline-primary"><i class="fa fa-fw fa-sm fa-plus"></i> <?= language()->heartbeats->create ?></a>
                <?php endif ?>
            </div>
            ...
            <div class="ml-3">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a href="<?= url('heartbeats?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item">
                            <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= language()->global->export_csv ?>
                        </a>
                        <a href="<?= url('heartbeats?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                    ...
                    <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                        <div class="dropdown-header d-flex justify-content-between">
                            ...
                            <?php if(count($data->filters->get)): ?>
                                <a href="<?= url('heartbeats') ?>" class="text-muted"><?= language()->global->filters->reset ?></a>
                            <?php endif ?>
                        </div>
                        ...
                        <form action="" method="get" role="form">
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="search_by" id="search_by" class="form-control form-control-sm">
                                    <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= language()->heartbeats->filters->search_by_name ?></option>
                                </select>
                            </div>
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="order_by" id="order_by" class="form-control form-control-sm">
                                    ...
                                    <option value="last_run_datetime" <?= $data->filters->order_by == 'last_run_datetime' ? 'selected="selected"' : null ?>><?= language()->heartbeats->filters->order_by_last_run_datetime ?></option>
                                    <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= language()->heartbeats->filters->order_by_name ?></option>
                                    <option value="uptime" <?= $data->filters->order_by == 'uptime' ? 'selected="selected"' : null ?>><?= language()->heartbeats->filters->order_by_uptime ?></option>
                                </select>
                            </div>
    ...
    <?php else: ?>
        ...
        <div class="d-flex flex-column align-items-center justify-content-center">
            <img src="<?= SITE_URL . ASSETS_URL_PATH . 'images/no_rows.svg' ?>" class="col-10 col-md-7 col-lg-5 mb-3" alt="<?= language()->heartbeats->no_data ?>" />
